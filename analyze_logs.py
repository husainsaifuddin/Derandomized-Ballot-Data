            ...
            for session in sessions:
                ...
                for card in cards:
                    ...
                    for contest in contests:
                        if contest.get("Id") == 1:  # Filter ContestId: 1
                            marks = contest.get("Marks", [])
                            for mark in marks:
                                candidate_id = mark.get("CandidateId")
                                if candidate_id in [59, 54]:  # Biden (59) or Trump (54)
                                    vote_data.append({
                                        "Timestamp": timestamp,
...
candidate_map = {59: "Joe Biden", 54: "Donald Trump"}
...
# Plot the data
colors = ['red', 'blue']  # Blue for Biden, Red for Trump
grouped.plot(kind='bar', figsize=(12, 6), width=0.8, color=colors)

# Customize the plot
plt.title("Vote Distribution by Hour (Biden vs. Trump)")
